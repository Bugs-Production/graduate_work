...
async def create_setup_intent(
    ...
    user_id = "637987f8-e99d-4b00-b4ca-54e377c042e2"  # заменить на реальный айди юзера, полученный с токена auth
...
async def stripe_webhook(
    request: Request, manage_service: SubscriptionManager = Depends(get_manager_service)
) -> JSONResponse:
    ...
    if data is not None:
        await manage_service.handle_webhook(event_type=event_type, data=data)
